  ...
  return (
    <div className="w-full max-w-lg mx-auto bg-surface/30 backdrop-blur-md rounded-3xl border border-white/10 p-6 shadow-2xl relative">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-4">
            <div className="text-white font-mono text-sm bg-black/30 px-3 py-1 rounded-lg border border-white/5">
                {common?.score}: <span className="text-primary font-bold">{score}</span>
            </div>
            <div className={`text-white font-mono text-sm px-3 py-1 rounded-lg border transition-all ${isNewRecord ? 'bg-yellow-500/20 border-yellow-500 text-yellow-300 animate-pulse' : 'bg-black/30 border-white/5'}`}>
                {common?.record}: <span className="font-bold">{highScore}</span>
            </div>
        </div>
      ...
      <div className="relative aspect-square bg-black/40 rounded-xl border border-white/20 overflow-hidden mx-auto max-w-[300px] sm:max-w-[400px] shadow-inner">
        ...
        {gameOver && (
          <div className="absolute inset-0 bg-black/80 backdrop-blur-md flex flex-col items-center justify-center z-20 animate-in fade-in duration-300">
            <h3 className="text-3xl font-bold text-red-500 mb-2">{common?.gameOver}</h3>
            
            {isNewRecord && (
                <span className="text-yellow-400 text-xs font-bold uppercase tracking-widest mb-2 animate-bounce">
                    {t?.minigames?.memory?.newRecord}
                </span>
            )}

            <p className="text-gray-300 mb-6 font-mono">{common?.score}: <span className="text-white font-bold text-xl">{score}</span></p>
      ...
      <div className="mt-8 grid grid-cols-3 gap-2 max-w-[180px] mx-auto md:hidden">
        <div />
        <button onTouchStart={(e) => { e.preventDefault(); if (direction !== 'DOWN') setDirection('UP'); }} onClick={() => direction !== 'DOWN' && setDirection('UP')} className="bg-white/10 p-4 rounded-2xl active:bg-primary active:text-bg transition-colors"><Icon icon="solar:arrow-up-bold" className="text-white text-xl" /></button>
        <div />
        <button onTouchStart={(e) => { e.preventDefault(); if (direction !== 'RIGHT') setDirection('LEFT'); }} onClick={() => direction !== 'RIGHT' && setDirection('LEFT')} className="bg-white/10 p-4 rounded-2xl active:bg-primary active:text-bg transition-colors"><Icon icon="solar:arrow-left-bold" className="text-white text-xl" /></button>
        <button onTouchStart={(e) => { e.preventDefault(); if (direction !== 'UP') setDirection('DOWN'); }} onClick={() => direction !== 'UP' && setDirection('DOWN')} className="bg-white/10 p-4 rounded-2xl active:bg-primary active:text-bg transition-colors"><Icon icon="solar:arrow-down-bold" className="text-white text-xl" /></button>
        <button onTouchStart={(e) => { e.preventDefault(); if (direction !== 'LEFT') setDirection('RIGHT'); }} onClick={() => direction !== 'LEFT' && setDirection('RIGHT')} className="bg-white/10 p-4 rounded-2xl active:bg-primary active:text-bg transition-colors"><Icon icon="solar:arrow-right-bold" className="text-white text-xl" /></button>
      </div>
