...
import CvModal from '../CvModal'; // Importando o novo Modal
...
export default function Footer() {
  ...
  const [isCvModalOpen, setIsCvModalOpen] = useState(false); // Estado do Modal CV
  ...
  return (
    <footer id="contact" className="bg-surface relative pt-20 mt-20 border-t border-white/5 flex flex-col items-center">
      <div className="max-w-4xl w-full mx-auto px-6 text-center pb-10">
        ...
        <div className="mb-12">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                <a href="mailto:felipetorresaraujo@gmail.com" className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-8 py-3 bg-primary text-bg font-bold rounded-full hover:bg-primary-600 transition-all hover:scale-105 shadow-[0_0_15px_rgba(var(--primary-rgb),0.3)]">
                    <Icon icon="solar:letter-bold" width="20" />
                    {t.footer.emailBtn}
                </a>
                
                {/* BOTAO ATUALIZADO PARA ABRIR O MODAL NO FOOTER TAMBÉM */}
                <button 
        ...
        <div className="relative flex flex-col items-center justify-center mb-10 group">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-4 p-1.5 md:p-2 bg-black/40 border border-white/10 rounded-2xl md:rounded-full backdrop-blur-md hover:border-primary/40 transition-all duration-500 shadow-inner group-hover:shadow-[0_0_30px_rgba(var(--primary-rgb),0.1)]">
            ...
            >
              ...
              <span className="absolute top-2 right-2 w-1.5 h-1.5 bg-yellow-400 rounded-full animate-pulse blur-[1px]"></span>
              <span className="absolute top-2 right-2 w-1.5 h-1.5 bg-yellow-300 rounded-full"></span>
            </button>
      ...
      <CvModal isOpen={isCvModalOpen} onClose={() => setIsCvModalOpen(false)} /> {/* Render do Modal CV */}
      <TechNews />
